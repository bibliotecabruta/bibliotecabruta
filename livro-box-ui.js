...
async function renderBoxRelations(){const id=new URLSearchParams(location.search).get('id');if(!id)return;const {data:book}=window.BB_BOOK_PROMISE?await window.BB_BOOK_PROMISE:await sbBoxUi.from('books').select('id,item_type').eq('id',id).maybeSingle();if(!book)return;const root=await waitForBookPage();if(!root)return;
