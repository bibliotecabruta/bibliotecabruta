...
async function renderBoxRelations(){const id=window.BB_BOOK_ID||new URLSearchParams(location.search).get('id')||document.body?.dataset.bookId||'';if(!id)return;let ctx=null;
 ...
 if(ctx){
  if(ctx.item_type==='box'){
   const data=ctx.box_contents||[];if(data.length){const section=document.createElement('section');section.className='detail-section';section.innerHTML=`<div class="eyebrow">BOX DA SÉRIE</div><h2>Livros incluídos neste box</h2><p class="muted">Este produto reúne os volumes abaixo, que continuam com suas fichas individuais no catálogo.</p><div class="series-volume-grid">${data.map(x=>boxBookCard(x.books,x.books?.series_volume?`VOLUME ${x.books.series_volume}`:'' )).join('')}</div>`;root.appendChild(section)}
  }else{
   const data=ctx.containing_boxes||[];if(data.length){const section=document.createElement('section');section.className='detail-section';section.innerHTML=`<h2>Também disponível em box</h2><div class="series-volume-grid">${data.map(x=>boxBookCard(x.boxes,'BOX DA SÉRIE')).join('')}</div>`;root.appendChild(section)}
  }
  return;
 }
 const {data:book}=window.BB_BOOK_PROMISE?await window.BB_BOOK_PROMISE:await sbBoxUi.from('books').select('id,item_type').eq('id',id).maybeSingle();if(!book)return;
