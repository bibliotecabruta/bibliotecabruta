...
function relThumb(url){const raw=String(url||'');if(!raw)return'';try{const u=new URL(raw,location.origin),p='/storage/v1/object/public/',file=u.pathname.split('/').pop()||'',base=file.replace(/\.[^.]+$/,''),bad=new Set(['3118e072-fc8b-40f6-b86b-2629742e7927','ad8f67fb-f1d3-46b9-a7b0-1cf9b5e1bb82']);if(u.pathname.startsWith('/assets/capas-edicoes/')&&base&&!bad.has(base))return new URL('/assets/capas-thumb/'+base+'.webp',u.origin).href;if(u.hostname.endsWith('.supabase.co')&&u.pathname.includes(p)){u.pathname=u.pathname.replace(p,'/storage/v1/render/image/public/');u.searchParams.set('width','260');u.searchParams.set('quality','82');u.searchParams.set('resize','contain')}return u.href}catch{return raw}}
...
async function injectRelated(){
 ...
 const currentArea=relPublicArea(b.area);
 const html=`<section class="detail-section related-books"><div class="author-more-head"><div><small>CONTINUE GARIMPANDO</small><h2>Livros por caminhos parecidos</h2><p class="muted">Outros autores que compartilham categorias ou temas deste livro.</p></div><a class="series-inline-link" href="${relAreaHref(b.area)}">Explorar ${relEsc(currentArea)} →</a></div><div class="author-more-grid">${related.map(x=>{const cover=relCover(x),why=(x.__matches||[]).slice(0,2).join(' • ');return `<a class="author-more-card" href="livro.html?id=${encodeURIComponent(x.id)}">${cover?`<img src="${relEsc(relThumb(cover))}" alt="Capa de ${relEsc(x.title)}" loading="lazy" decoding="async" onerror="this.remove()">`:'<div class="mini-placeholder">Sem capa</div>'}<strong>${relEsc(x.title)}</strong><span>${relEsc(x.authors?.name||'Autor não informado')}</span>${why?`<small>${relEsc(why)}</small>`:''}</a>`}).join('')}</div></section>`;
