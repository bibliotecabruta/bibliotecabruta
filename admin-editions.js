...
function editionEffectiveCover(e){
 if(!e)return '';
 return editionPrimaryVariant(e)?.cover_url||e.cover_url||'';
}
...
function editEditionAdmin(id){
 ...
 f.elements.is_primary_eligible.checked=e.is_primary_eligible!==false;
...
async function saveEditionAdmin(ev){
 ...
 try{
   ...
   const existingEdition=editionId?adminEditionRows.find(x=>x.id===editionId):null;
   const primaryEligible=fd.get('is_primary_eligible')==='on';
   if(existingEdition?.is_primary&&!primaryEligible)throw new Error('Escolha outra edição como principal antes de marcar esta como componente de uma edição multi-volume.');
   if(fd.get('make_primary')==='on'&&!primaryEligible)throw new Error('Uma edição marcada como componente não pode ser a edição principal geral.');
   ...
   const payload={
     ...
     is_primary_eligible:primaryEligible
   };
   ...
   const currentPrimary=adminEditionRows.find(x=>x.is_primary);
   const savedYear=payload.publication_year;
   const savedEdition=editionId?adminEditionRows.find(x=>x.id===editionId):null;
   const hasUsableCover=!!(newCover||editionEffectiveCover(savedEdition));
   const autoNewest=primaryEligible&&hasUsableCover&&savedYear!=null&&(!currentPrimary?.publication_year||savedYear>currentPrimary.publication_year);
   const shouldPrimary=primaryEligible&&(fd.get('make_primary')==='on'||!currentPrimary||autoNewest);
   if(shouldPrimary){
...
async function saveVariantAdmin(ev){
 ...
 try{
   ...
   const currentPrimary=adminEditionRows.find(x=>x.is_primary);
   const autoNewestEdition=e.is_primary_eligible!==false&&!e.is_primary&&e.publication_year!=null&&(!currentPrimary?.publication_year||e.publication_year>currentPrimary.publication_year);
   if(autoNewestEdition){
     const {error}=await sbAdmin.rpc('set_primary_edition',{p_edition_id:editionId});
     if(error)throw error;
   }
   await loadEditionAdmin(adminEditionBookId);
   syncPrimaryEditionAdmin();
...
async function setPrimaryEditionAdmin(id){
 const edition=adminEditionRows.find(x=>x.id===id);
 if(!edition){msg('Edição não encontrada.','error');return}
 if(edition.is_primary_eligible===false){msg('Esta edição está marcada como componente de uma edição multi-volume e não pode virar a principal geral.','error');return}
 if(!editionEffectiveCover(edition)){msg('Cadastre uma capa para esta edição antes de torná-la a edição principal geral.','error');return}
 try{
