...
async function saveEditionAdmin(ev){
 ...
 try{
   ...
   const autoNewest=hasUsableCover&&savedYear!=null&&(!currentPrimary?.publication_year||savedYear>currentPrimary.publication_year);
   const shouldPrimary=fd.get('make_primary')==='on'||!currentPrimary||autoNewest;
   if(shouldPrimary){
...
async function saveVariantAdmin(ev){
 ...
 try{
   ...
   const currentPrimary=adminEditionRows.find(x=>x.is_primary);
   const autoNewestEdition=!e.is_primary&&e.publication_year!=null&&(!currentPrimary?.publication_year||e.publication_year>currentPrimary.publication_year);
