...
const bookSelect='id,title,original_title,original_year,synopsis,area,series_volume,cover_url,authors(id,name),series(id,name),editions(country,publisher,publication_year,isbn,pages,translator,is_primary,cover_url,notes,edition_label,binding),book_categories(categories(id,name)),book_tags(tags(id,name))';
...
for(const b of books){
 const brEditions=(b.editions||[]).filter(e=>e.country==='Brasil').sort((a,z)=>(z.is_primary?1:0)-(a.is_primary?1:0)||(z.publication_year||0)-(a.publication_year||0));
