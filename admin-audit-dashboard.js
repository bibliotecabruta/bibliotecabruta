...
function isLocalAuditCover(url){if(!url)return false;try{return new URL(url,location.href).origin===location.origin}catch{return false}}
function auditCoverHosting(){let local=0,external=0,missing=0;for(const b of auditDashState.books){if(!b.cover_url){missing++;continue}if(isLocalAuditCover(b.cover_url))local++;else external++}return{local,external,missing,total:auditDashState.books.length}}
...
function renderAuditDashboard(){
  ...
  root.innerHTML=`
    ...
    <div class="audit-kpis">
      ...
      <button class="audit-kpi ${ch.external?'warn':'ok'}" onclick="setAuditTab('capas','__external__')"><strong>${ch.external}</strong><span>Capas externas</span></button>
...
function renderAuditCovers(box){
 ...
 else if(auditDashState.filter==='__external__')rows=rows.filter(x=>x.book.cover_url&&!isLocalAuditCover(x.book.cover_url));
 else if(auditDashState.filter==='__local__')rows=rows.filter(x=>x.book.cover_url&&isLocalAuditCover(x.book.cover_url));
 ...
 box.innerHTML=`<div class="audit-cover-stats"><span class="audit-tag ok">OK · ${ok}</span><span class="audit-tag">Baixa resolução · ${low}</span><span class="audit-tag">Quebradas · ${broken}</span><span class="audit-tag neutral">Não verificadas / antigas · ${unknown}</span><span class="audit-tag ok">Locais · ${hosting.local}</span><span class="audit-tag">Externas · ${hosting.external}</span><span class="audit-tag neutral">Sem capa · ${hosting.missing}</span></div><div class="audit-toolbar"><input type="search" placeholder="Pesquisar título ou autor…" oninput="setAuditQuery(this.value)"><select onchange="setAuditFilter(this.value)"><option value="__problems__" ${auditDashState.filter==='__problems__'?'selected':''}>Problemáticas</option><option value="__unknown__" ${auditDashState.filter==='__unknown__'?'selected':''}>Não verificadas / antigas</option><option value="__external__" ${auditDashState.filter==='__external__'?'selected':''}>Capas externas</option><option value="__local__" ${auditDashState.filter==='__local__'?'selected':''}>Capas locais</option><option value="__missing__" ${auditDashState.filter==='__missing__'?'selected':''}>Sem capa</option><option value="baixa_resolucao" ${auditDashState.filter==='baixa_resolucao'?'selected':''}>Baixa resolução</option><option value="quebrada" ${auditDashState.filter==='quebrada'?'selected':''}>Quebradas</option><option value="" ${auditDashState.filter===''?'selected':''}>Todas com capa</option></select><button class="secondary" onclick="scanAuditCovers(false,50,false)">Verificar próximas 50</button></div><p class="audit-section-note">A auditoria carrega a imagem real e separa hospedagem local de hotlinks externos. Consideramos baixa resolução quando a capa tem menos de 600 px de largura ou 900 px de altura. Resultados ficam em cache por 30 dias e são invalidados se a URL mudar.</p><div class="audit-list">${rows.length?rows.map(coverAuditRow).join(''):'<div class="audit-empty">Nenhuma capa neste filtro.</div>'}</div>`;
}
function coverAuditRow(x){const b=x.book,c=x.cache,current=c&&c.cover_url===b.cover_url;if(!b.cover_url)return `<div class="audit-row"><div class="audit-row-main"><strong>${esc(b.title)}</strong><small>${esc(b.author||'—')}</small><div class="audit-tags"><span class="audit-tag">Sem capa</span></div></div><div class="audit-row-meta">Nenhuma URL de capa cadastrada</div><div class="audit-row-actions"><button class="secondary" onclick="editBook('${b.id}')">Editar livro</button></div></div>`;let label='<span class="audit-tag neutral">Não verificada</span>',meta='';if(current){if(c.status==='ok')label='<span class="audit-tag ok">OK</span>';if(c.status==='baixa_resolucao')label='<span class="audit-tag">Baixa resolução</span>';if(c.status==='quebrada')label='<span class="audit-tag">Quebrada</span>';meta=c.width&&c.height?`${c.width}×${c.height}px`:''}const hosting=isLocalAuditCover(b.cover_url)?'<span class="audit-tag ok">Local</span>':'<span class="audit-tag">Externa</span>';return `<div class="audit-row"><div class="audit-row-main"><strong><img class="audit-cover-thumb" src="${esc(b.cover_url)}" alt="" loading="lazy" onerror="this.style.visibility='hidden'">${esc(b.title)}</strong><small>${esc(b.author||'—')}</small><div class="audit-tags">${label}${hosting}</div></div><div class="audit-row-meta">${esc(meta||b.cover_url)}</div><div class="audit-row-actions"><button class="secondary" onclick="scanSingleAuditCover('${b.id}')">Verificar</button><button class="secondary" onclick="editBook('${b.id}')">Editar livro</button></div></div>`}
