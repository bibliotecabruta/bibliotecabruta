...
function auditCoverLocation(url){if(!url)return'missing';try{const u=new URL(url,location.href);if(u.origin===location.origin)return'repo';if(u.hostname==='mjlujvpvpxelpcbxgleo.supabase.co')return'supabase';return'third_party'}catch{return'third_party'}}
function isLocalAuditCover(url){return auditCoverLocation(url)==='repo'}
function auditCoverHosting(){let repo=0,supabase=0,thirdParty=0,missing=0;for(const b of auditDashState.books){const kind=auditCoverLocation(b.cover_url);if(kind==='repo')repo++;else if(kind==='supabase')supabase++;else if(kind==='third_party')thirdParty++;else missing++}return{repo,supabase,thirdParty,offRepo:supabase+thirdParty,missing,total:auditDashState.books.length}}
...
function renderAuditDashboard(){
  ...
  const bp=auditBookPendingCount(),sp=auditSeriesIssueCount(),si=auditSeriesIncompleteCount(),ap=auditAuthorPendingCount(),cp=auditCoverProblemCount(),cc=auditCoverCoverage(),ch=auditCoverHosting(),ip=auditIntegrityCount(),lp=auditDashState.links.length;
  root.innerHTML=`
    ...
    <div class="audit-kpis">
      ...
      <button class="audit-kpi ${ch.offRepo?'warn':'ok'}" onclick="setAuditTab('capas','__offrepo__')"><strong>${ch.offRepo}</strong><span>Fora do repositório · ${ch.thirdParty} terceiros</span></button>
...
function bookAuditRow(b){const issues=b.issues||[],tags=issues.map(i=>`<span class="audit-tag">${esc(ADMIN_ISSUE_LABELS[i]||i)}</span>`).join(''),editionAlert=issues.includes('edicao_recente_sem_capa')||issues.includes('edicao_principal_desatualizada'),currentEdition=[b.publisher,b.brazil_year,b.isbn].filter(Boolean).join(' · ')||'Edição principal incompleta',latestEdition=[b.latest_publisher,b.latest_brazil_year,b.latest_isbn].filter(Boolean).join(' · ');return `<div class="audit-row"><div class="audit-row-main"><strong>${esc(b.title)}</strong><small>${esc(b.author||'Autor não informado')}${b.original_title?' · '+esc(b.original_title):''}</small><div class="audit-tags">${tags||'<span class="audit-tag ok">Sem pendências</span>'}</div></div><div class="audit-row-meta">${esc(b.area||'—')}<br><strong>Principal:</strong> ${esc(currentEdition)}${editionAlert&&latestEdition?`<br><strong>Mais recente:</strong> ${esc(latestEdition)}${issues.includes('edicao_recente_sem_capa')?' · falta capa':''}`:''}</div><div class="audit-row-actions"><button class="secondary" type="button" onclick="editBook('${b.id}')">Editar livro</button><a class="secondary" href="livro.html?id=${encodeURIComponent(b.id)}" target="_blank" rel="noopener">Ver ficha</a></div></div>`}
...
function renderAuditCovers(box){
 const cache=currentCoverCacheMap(),hosting=auditCoverHosting();
 let rows=auditDashState.books.map(b=>({book:b,cache:cache.get(b.id)})).filter(x=>auditTextMatch(x.book.title,x.book.author,x.book.cover_url));
 if(auditDashState.filter==='__problems__')rows=rows.filter(x=>x.book.cover_url&&x.cache&&x.cache.cover_url===x.book.cover_url&&x.cache.status!=='ok');
 else if(auditDashState.filter==='__unknown__')rows=rows.filter(x=>x.book.cover_url&&!isCoverCacheCurrent(x.book,x.cache));
 else if(auditDashState.filter==='__offrepo__')rows=rows.filter(x=>['supabase','third_party'].includes(auditCoverLocation(x.book.cover_url)));
 else if(auditDashState.filter==='__thirdparty__')rows=rows.filter(x=>auditCoverLocation(x.book.cover_url)==='third_party');
 else if(auditDashState.filter==='__supabase__')rows=rows.filter(x=>auditCoverLocation(x.book.cover_url)==='supabase');
 else if(auditDashState.filter==='__local__')rows=rows.filter(x=>auditCoverLocation(x.book.cover_url)==='repo');
 else if(auditDashState.filter==='__missing__')rows=rows.filter(x=>!x.book.cover_url);
 ...
 else rows=rows.filter(x=>x.book.cover_url);
 ...
 box.innerHTML=`<div class="audit-cover-stats"><span class="audit-tag ok">OK · ${ok}</span><span class="audit-tag">Baixa resolução · ${low}</span><span class="audit-tag">Quebradas · ${broken}</span><span class="audit-tag neutral">Não verificadas / antigas · ${unknown}</span><span class="audit-tag ok">Repositório · ${hosting.repo}</span><span class="audit-tag neutral">Nosso Supabase · ${hosting.supabase}</span><span class="audit-tag">Terceiros · ${hosting.thirdParty}</span><span class="audit-tag neutral">Sem capa · ${hosting.missing}</span></div><div class="audit-toolbar"><input type="search" placeholder="Pesquisar título ou autor…" oninput="setAuditQuery(this.value)"><select onchange="setAuditFilter(this.value)"><option value="__problems__" ${auditDashState.filter==='__problems__'?'selected':''}>Problemáticas</option><option value="__unknown__" ${auditDashState.filter==='__unknown__'?'selected':''}>Não verificadas / antigas</option><option value="__offrepo__" ${auditDashState.filter==='__offrepo__'?'selected':''}>Fora do repositório</option><option value="__thirdparty__" ${auditDashState.filter==='__thirdparty__'?'selected':''}>Hotlinks de terceiros</option><option value="__supabase__" ${auditDashState.filter==='__supabase__'?'selected':''}>Nosso Supabase</option><option value="__local__" ${auditDashState.filter==='__local__'?'selected':''}>No repositório</option><option value="__missing__" ${auditDashState.filter==='__missing__'?'selected':''}>Sem capa</option><option value="baixa_resolucao" ${auditDashState.filter==='baixa_resolucao'?'selected':''}>Baixa resolução</option><option value="quebrada" ${auditDashState.filter==='quebrada'?'selected':''}>Quebradas</option><option value="" ${auditDashState.filter===''?'selected':''}>Todas com capa</option></select><button class="secondary" onclick="scanAuditCovers(false,50,false)">Verificar próximas 50</button></div><p class="audit-section-note">A auditoria carrega a imagem real e separa capas no repositório, no nosso Supabase e hotlinks de terceiros. Consideramos baixa resolução quando a capa tem menos de 600 px de largura ou 900 px de altura. Resultados ficam em cache por 30 dias e são invalidados se a URL mudar.</p><div class="audit-list">${rows.length?rows.map(coverAuditRow).join(''):'<div class="audit-empty">Nenhuma capa neste filtro.</div>'}</div>`;
}
function coverAuditRow(x){const b=x.book,c=x.cache,current=c&&c.cover_url===b.cover_url;if(!b.cover_url)return `<div class="audit-row"><div class="audit-row-main"><strong>${esc(b.title)}</strong><small>${esc(b.author||'—')}</small><div class="audit-tags"><span class="audit-tag">Sem capa</span></div></div><div class="audit-row-meta">Nenhuma URL de capa cadastrada</div><div class="audit-row-actions"><button class="secondary" onclick="editBook('${b.id}')">Editar livro</button></div></div>`;let label='<span class="audit-tag neutral">Não verificada</span>',meta='';if(current){if(c.status==='ok')label='<span class="audit-tag ok">OK</span>';if(c.status==='baixa_resolucao')label='<span class="audit-tag">Baixa resolução</span>';if(c.status==='quebrada')label='<span class="audit-tag">Quebrada</span>';meta=c.width&&c.height?`${c.width}×${c.height}px`:''}const locationKind=auditCoverLocation(b.cover_url),hosting=locationKind==='repo'?'<span class="audit-tag ok">Repositório</span>':locationKind==='supabase'?'<span class="audit-tag neutral">Supabase</span>':'<span class="audit-tag">Terceiro</span>';return `<div class="audit-row"><div class="audit-row-main"><strong><img class="audit-cover-thumb" src="${esc(b.cover_url)}" alt="" loading="lazy" onerror="this.style.visibility='hidden'">${esc(b.title)}</strong><small>${esc(b.author||'—')}</small><div class="audit-tags">${label}${hosting}</div></div><div class="audit-row-meta">${esc(meta||b.cover_url)}</div><div class="audit-row-actions"><button class="secondary" onclick="scanSingleAuditCover('${b.id}')">Verificar</button><button class="secondary" onclick="editBook('${b.id}')">Editar livro</button></div></div>`}
