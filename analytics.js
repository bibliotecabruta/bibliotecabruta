...
window.bbAnalyticsConsent={
 ...
 reset(){try{localStorage.removeItem(CONSENT_KEY)}catch{}}
};
...
function ensureConsentUi(){
 ...
 style.id='bbAnalyticsConsentStyle';
 ...
 box.id='bbAnalyticsConsent';
 box.className='bb-analytics-consent';
 box.setAttribute('role','dialog');
 box.setAttribute('aria-label','Preferências de métricas');
 box.innerHTML='<p><strong>Métricas do site.</strong> A Biblioteca Bruta usa Google Analytics para entender quais páginas, buscas e filtros são mais úteis. Nenhuma tag do Google Analytics é carregada antes do seu aceite. <a href="privacidade.html" style="color:#e1b86d">Saiba mais</a>.</p><div class="bb-analytics-actions"><button type="button" class="bb-analytics-decline">Recusar</button><button type="button" class="bb-analytics-accept">Aceitar métricas</button></div>';
 document.body.appendChild(box);
 box.querySelector('.bb-analytics-accept').addEventListener('click',()=>{
  saveConsent('granted');
  box.remove();
  injectGtag();
  track('analytics_consent',{choice:'granted'});
 });
 box.querySelector('.bb-analytics-decline').addEventListener('click',()=>{
  saveConsent('denied');
  box.remove();
 });
}
 ...
 if(catalog){
  catalog.addEventListener('input',()=>{
   ...
   searchTimer=setTimeout(()=>{
    const q=clean(catalog.value,80);
    if(q.length>=2&&q!==lastSearch){lastSearch=q;track('search',{search_term:q,search_location:'catalog'})}
   },900);
  });
 }
}
