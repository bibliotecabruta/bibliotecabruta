...
function clearRefinementsKeepSearch(){
 ...
 ['filterArea','filterCategory','filterTag','filterAuthor','filterSeries','filterStatus'].forEach(id=>{const e=document.getElementById(id);if(e)e.value=''});
...
function activeRefineCount(f=getFilters()){return['area','category','tag','author','series','status'].filter(k=>f[k]).length+(f.added?1:0)+(f.standalone?1:0)}
