...
function latestBookCard(b){
 ...
 const coverHtml=cover?`<img src="${esc(cover)}" alt="Capa brasileira de ${esc(b.title)}" loading="lazy" onerror="this.onerror=null;this.parentElement.textContent=this.alt.replace('Capa brasileira de ','')">`:esc(b.title);
...
async function renderNews(){
 ...
 target.innerHTML=data.map(n=>{const image=safeUrl(n.image_url),source=safeUrl(n.source_url),purchase=safeUrl(n.purchase_url),summary=n.summary||String(n.body||'').replace(/[#*\[\]()\-]/g,'').slice(0,220),href=n.slug?'noticia.html?slug='+encodeURIComponent(n.slug):'noticia.html?id='+encodeURIComponent(n.id);return '<article class="news-item '+(n.is_featured?'news-home-featured':'')+'">'+(image?'<a class="news-image" href="'+href+'"><img src="'+esc(image)+'" alt="'+esc(n.image_alt||n.title)+'" loading="lazy"></a>':'')+'<div class="news-content"><small>'+(n.is_featured?'DESTAQUE • ':'')+new Date(n.published_at).toLocaleDateString("pt-BR")+'</small><h3><a href="'+href+'">'+esc(n.title)+'</a></h3><p>'+newsText(summary)+(!n.summary&&String(n.body||'').length>220?'…':'')+'</p><div class="news-actions"><a class="news-read" href="'+href+'">Leia mais</a>'+(source?'<a href="'+esc(source)+'" target="_blank" rel="noopener noreferrer">'+esc(n.source_label||'Fonte')+'</a>':'')+(purchase?'<a class="news-buy" href="'+esc(purchase)+'" target="_blank" rel="'+(n.purchase_is_affiliate?'sponsored nofollow noopener noreferrer':'noopener noreferrer')+'">'+esc(n.purchase_label||'Comprar')+'</a>':'')+'</div></div></article>'}).join("");
}
