...
async function loadBooks(area="",query="",limit=0){
  let req=sb.from("books").select(`id,title,original_title,area,original_year,cover_url,series_volume,series_id,created_at,authors(id,name),series(id,name),book_authors(author_order,is_primary,authors(id,name)),book_series(series_volume,is_primary,series(id,name)),book_categories(categories(name)),book_tags(tags(name)),editions(publisher,publication_year,is_primary,country)`).order("created_at",{ascending:false});
  ...
  const q=(query||"").trim().toLowerCase();let rows=data||[];
